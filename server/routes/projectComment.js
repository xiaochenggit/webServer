...
router.post('/create', (req, res, next) => {
	...
	if (cookieUser) {
		if (!cId) { // 一级回复
			...
			let newOpinion = new ProjectComment(opinion);
			...
			ProjectComment.findOne({_id: cId}, (err, opinion) => {
        if (err) {
...
router.get('/list', (req, res, next) => {
	...
	ProjectComment.find({project: typeId})
...
router.post('/delete', (req, res, next) => {
	...
	if (cookieUser) {
		...
		ProjectComment.findOne({_id: id})
		.populate({path: "project", "select": 'user'})
		.exec((err, opinion) => {
			if (err) {
				res.json({ status: 401, msg: err.message });
			} else {
				if (opinion) { // 是否存在
					if (!replyId) { // 一级
						...
						opinion.project.user == cookieUser._id) { // 权限判断
							ProjectComment.remove({_id: id}, () => {
						...
						opinion.reply.forEach((item, index) => { // 找到二级
							if (item._id == replyId) {
								...
								opinion.project.user == cookieUser._id ) { // 判断权限
